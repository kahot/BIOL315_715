#Tidy Data: Week 3 Assignment and more
...
colnames(het)[2:3]<-c("type","heterozygosity")

# Copy your output (table) here #
het
#            Population     type heterozygosity
...
shapiro.test(het$heterozygosity)
shapiro.test(log(het$heterozygosity))
shapiro.test(sqrt(het$heterozygosity))

cor.test(data1$Expected,data1$Observed, method="spearman")

wilcox.test(data1$Expected,data1$Observed)


ggplot(het, aes(x=Population, y=heterozygosity, color=type))+
    geom_point()
...
ggplot(iris_reshaped, aes(x=Species, y=value, color=variable))+
    geom_boxplot()
...
anosim_re<-anosim(iris2, iris$Species,permutations=1000)
anosim_re
